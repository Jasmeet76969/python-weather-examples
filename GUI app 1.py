...
def get_weather():
    ...
    if "message" in weather_data:
        ...
        weather_text +=f"Temperature:{weather_data['main']['temp']}°C\n"
        weather_text +=f"Pressure:{weather_data['main']['pressure']}hPa\n"
        weather_text +=f"Humidity:{weather_data['main']['humidity']}%\n"
        weather_text +=f"MinTemp:{weather_data['main']['temp_min']}°C\n"
        weather_text +=f"MaxTemp:{weather_data['main']['temp_max']}°C\n"
        ...
        weather_text += f"Sunrise: {sunrise}\n"
        weather_text += f"Sunset: {sunset}\n"
...
city_entry.insert(0,"Gobindgarh")
...
weather_frame=tk.Frame(root,bg='pink',bd=10)
...
icon_frame=tk.Frame(weather_frame,bg='lightblue')
